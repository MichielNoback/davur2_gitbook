...
After grouping on a variable, you will sometimes be interested in the top or bottom _n_ rows. The `head()` and `slice()` functions will not work then, surprisingly enough. Suppose you want the top 2 responses from both the males and females in the dose100mg group. Surprisingly enough this does not work:

```{r need-for-row-number1}
dose_response_long %>%
...
Instead, to select the top _n_ values after grouping you need to use `filter(row_number() <some-logical-test>)`. Here is the correct solution.
