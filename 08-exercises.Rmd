...
Create a scatter plotvisualization of the `Puromycin` dataset. Color by `state` and add a loess model smoother without error margin. Try to use the `formula = ` parameter for `geom_smooth()` for getting a better regression line.
...
1. **Starting weight**. Which chick had the highest starting weight? List this chick, together with its diet as follows: <Chick> <Diet> <weight>.

2. **Total weight gain**. Determine the total weight gain for each chick and list all chickens together with their diet and number of weight measurements. Order from high to low weight gain.
