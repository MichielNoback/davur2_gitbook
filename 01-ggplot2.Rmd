...
There are some statistics available to adjust what is shown on the y axis. The default that is used by `geom_histogram` is `..count..`, but if you want it scaled to a maximum of 1, use `..ncount..`

```{r histogram2, fig.asp=.75, out.width='80%', fig.align='center'}
ggplot(data=faithful, mapping = aes(x = waiting)) + 
    geom_histogram(binwidth = 3, aes(y=..ncount..))
```

If you want proportions, you could do this:

```{r histogram3, fig.asp=.75, out.width='80%', fig.align='center'}
ggplot(data=faithful, mapping = aes(x = waiting)) + 
    geom_histogram(binwidth = 3, aes(y=..count.. / sum(..count..)))
```
...
```{r inheritace, fig.asp=.75, out.width='80%', fig.align='center'}
...
```{r facetting, fig.asp=.75, out.width='80%', fig.align='center'}
ggplot(data = airquality, mapping = aes(x = Solar.R, y = Ozone)) +
    geom_smooth(aes(linetype = Month), method="lm") +
...
Scales and Coordinates are used to adjust the way your data is mapped and displayed. Here, a log10 scale is applied to the y axis and the x axis is reversed.
