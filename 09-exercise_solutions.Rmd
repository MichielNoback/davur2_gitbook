...
```{r puromycin1, fig.asp=0.7, out.width='80%', fig.align='center', fig.cap="reaction velocity versus substrate concentration"}
...
Maybe it is better to do try to find another smoothing function? 
...
In my opinion, the boxplot is an excellent choice for the visualization. `geom_jitter()` could also work but is more of a hassle to split out on Dose.
...
```{r chick-starting-weight, results = 'hold'}
ChickWeight %>% 
    filter(Time == 0) %>%
    arrange(desc(weight)) %>%
...
```{r chick-weight-gain, results = 'hold'}
ChickWeight %>% 
    group_by(Chick) %>%
...
```{r chick-diet-weight-gain, results = 'hold'}
ChickWeight %>% 
    group_by(Chick) %>%
    mutate(weight_gain = weight - lag(weight)) %>%
    summarize(timepoints = n(), 
...
Load the data using the `lubridate` package so that the Date column becomes a date type.
...
```{r births-month-barplot2, fig.asp=0.7, out.width='70%', fig.align='center', fig.cap="Dayly birth counts"}
library(ggplot2)
female_births %>%
